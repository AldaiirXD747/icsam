...
// Function to cleanMatchesOnly but keep teams and championships
export const cleanMatchesOnly = async () => {
  try {
    ...
    console.log("Limpeza de dados de partidas concluída! Times e campeonatos foram mantidos.");
    return { success: true, message: "Todos os dados de partidas, estatísticas e classificações foram removidos com sucesso. Times e campeonatos foram mantidos." };
  } catch (error) {
    console.error("Erro durante a limpeza de dados:", error);
    return { success: false, error: error instanceof Error ? error.message : 'Erro desconhecido' };
  }
